...
覆盖技术：划分程序功能模块、不会同时执行的模块共享同一块内存。
程序开发者的主要工作：模块划分、确定模块的共享关系。


2. 什么是交换技术？覆盖与交换有什么不同？

交换技术：作为一个整体的进程地址空间的换入和换出。
不同：覆盖是通过切块，解决进程空间大于物理内存的问题；交换是通过进程整体交换，解决高优先级进程运行需要的内存空间不够问题。

3. 覆盖和交换技术在现代计算机系统中还有需要吗？可能用在什么地方？

由于现代计算机采用虚拟内存的方式，所以覆盖技术不需要。但是交换技术还可以用到。
...
保护位：表示允许页面访问方式。
 ...
 作业全部进入辅存，运转时，并不把整个作业程序一起都装入到内存，只装入目前要用的若干页。
 运行过程中，虚拟地址被转换成（页号，页内偏移）。
 根据页号查页表，如果该页不在内存中，就没有具体的块号与之对应，表明“缺页”，运行无法继续，此时，要根据页号把它从辅存中调入内存。
