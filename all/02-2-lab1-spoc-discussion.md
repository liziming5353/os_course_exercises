...
栈内依次存储保留寄存器、函数参数、返回地址、局部变量、临时参数等，通过push和pop操作实现函数调用。
...
ebp可以直接获得，若不内联则会得到错误的ebp值。而由于没有直接获取eip值的指令，我们需要利用call指令将eip压栈的特性，通过调用read_eip函数来读出压在栈上的eip的值。若将read_eip内联，则不会有函数调用存在，无法获得eip的值。
